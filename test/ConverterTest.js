...
describe('Converter', function() {
  ...
  describe('convert', function() {
    ...
    it('should convert markdown with showdown extension to react elements', function() {
      ...
      var actualHtml = renderToStaticMarkup(reactElement);
      var expectedHtml =
        '<p>Hello <a href="http://twitter.com/jerolimov">@jerolimov</a></p>';
      assert.equal(actualHtml, expectedHtml);
    });
    ...
    it('should convert markdown with react component tag to react elements without children', function() {
      ...
      var reactElement = converter.convert(
    ...
    it('should convert markdown to table tags', function() {
      ...
      var actualHtml = renderToStaticMarkup(reactElement);
      var expectedHtml =
        '<table><thead><tr><th>h1</th><th>h2</th><th>h3</th></tr></thead><tbody><tr><td><em>foo</em></td><td><strong>bar</strong></td><td>baz</td></tr></tbody></table>';
      assert.equal(actualHtml, expectedHtml);
    });
    ...
    it('should strip the markdown prop', function() {
      var converter = new Converter({components: {MyComponent: MyComponent}});
      var reactElement = converter.convert(
        '<MyComponent markdown="1" tag="strong"/>'
      );
      assert.deepEqual(reactElement.props.children[0].props, {
        tag: 'strong',
        children: null
      });
    });
    ...
    it('should handle class attributes of inline html', function() {
      var converter = new Converter({components: {MyComponent: MyComponent}});
      var reactElement = converter.convert(
        '<span class="foo">text</span><MyComponent markdown="1" tag="strong"/>'
      );
      var actualHtml = renderToStaticMarkup(reactElement);
      var expectedHtml =
        '<p><span class="foo">text</span><strong></strong></p>';
      assert.equal(actualHtml, expectedHtml);
    });
  });
});
